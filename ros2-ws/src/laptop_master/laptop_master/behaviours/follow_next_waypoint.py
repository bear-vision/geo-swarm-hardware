...
class FollowNextWaypoint(py_trees.behaviour.Behaviour):
    def __init__(self, behaviour_name, blackboard_waypoint_key):
        ...
        self.blackboard.register_key("waypoint_index", access=py_trees.common.Access.READ)
    ...
    def update(self) -> Status:
        ...
        Returns:
            ...
            Status.SUCCESS if the current waypoint was reached (dictated by blackboard.waypoint_index)
        ...
        # if self.blackboard.waypoint_index >= len(self.waypoints) or self.blackboard.finished_circle_layer:
        ...
        if not self._get_result_future:
            self.logger.info(f"Still waiting for waypoint #{self.blackboard.waypoint_index + 1} of {len(self.waypoints)}")
            return Status.RUNNING
        
        if self.action_result is not None:
            if self.action_result.success:
                self.logger.info(f"Reached waypoint #{self.blackboard.waypoint_index + 1} of {len(self.waypoints)}")
                self.blackboard.waypoint_index += 1
                # if self.blackboard.waypoint_index >= len(self.blackboard.waypoints):
                #     self.blackboard.finished_circle_layer = True
                self._send_goal_future = None
                self._get_result_future = None
                self.action_result = None
                ...
                self.logger.error(f"Failed to reach waypoint {self.blackboard.waypoint_index + 1}")
                return Status.FAILURE
    ...
    def send_waypoint(self):
        ...
        goal_msg.waypoint = self.waypoints[self.blackboard.waypoint_index]
        self._send_goal_future = self.action_client.send_goal_async(goal_msg)
        self._send_goal_future.add_done_callback(self.goal_response_callback)
        self.logger.info(f"Sending goal for waypoint #{self.blackboard.waypoint_index}: "
    ...
    def goal_response_callback(self, future) -> None:
        ...
        if not goal_handle.accepted:
            self.logger.error(f"Goal for waypoint #{self.blackboard.waypoint_index} was rejected")
            self._get_result_future = None
